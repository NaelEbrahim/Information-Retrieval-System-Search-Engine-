...
#         w2v_service = self.w2v_service.get_word2vec_service(self.dataset_name)
#         self.matrix = w2v_service.get_doc_vectors()
#         self.doc_ids = [str(i) for i in range(self.matrix.shape[0])]
#         self.doc_id_to_index = {doc_id: idx for idx, doc_id in enumerate(self.doc_ids)}
#
#     def build_faiss_index(self):
#         d = self.matrix.shape[1]  # dimension
#         self.index = faiss.IndexFlatL2(d)
#         self.index.add(self.matrix)
#         faiss.write_index(self.index, 'database/index_files/' + self.dataset_name + '/faiss.index')
#
#     def load_index(self, dataset_name):
#         self.index = faiss.read_index('database/index_files/' + self.dataset_name + '/faiss.index')
#
#
#     def search(self, query_vector, top_n=10):
#         D, I = self.index.search(np.array([query_vector]), top_n)
#         return [(self.doc_ids[idx], float(D[0][i])) for i, idx in enumerate(I[0])]
